...
double _VAR_RR_(double *Y_,  double *X_, int *para, double ETAK_, int DEBIAS_, double ALPHA_, int ISNULLETA_)
{
	// dimensions
	double _sigma2;
	int n     = para[0];
	...
	double **x = (double**)malloc(sizeof(double*)*n);
  	for (int i = 0; i < n; ++i) x[i] = &X_[i*p];
	double eta;

  	if (ISNULLETA_ == 1) {
  		eta = setup_eta(Y_, x, &ALPHA_, para);
  	} else {
  		eta = ETAK_;
  	}
	
  	var_rr(Y_, x, eta, para, &_sigma2, &DEBIAS_);
  	free(x);
  	return _sigma2;
}
...
double _VAR_MM_(double *y, double *X_, int *para, double *SIGMA_, int IDENTITY_, int ISNULL_)
{
	// dimensions
	double _sigma2;
	int n     = para[0];
    ...
    if (ISNULL_ == 0) {
        double temp1;
        for (i = 0; i < p; ++i) {
            for (temp1 = j = 0; j < p; ++j) temp1 += SIGMA_[j*p+i]*temp[j];
            xynorm += temp1*temp[i];
        }
        _sigma2 = ((p+n+1)*ynorm - xynorm/n)/(n+1);
  	} else {
        if (IDENTITY_ != 0) {
            ...
            _sigma2 = ((p+n+1)*ynorm - xynorm/n)/(n+1);
  			...
  			m1 /= p;
  			_sigma2 = (1+p*pow(m1, 2)/m2/(n+1))*ynorm - m1*xynorm/n/m2/(n+1);
  			freematrix(xtx, n);
  			free(lam);
  		}
  	}
...
double _VAR_MLE_(double *y, double *X_, int *para, int MAXITER_, double TOL_)
{
	// dimensions
	int n     = para[0];
	int p     = para[1];
	...
	while(step <MAXITER_) {
		...
		if (likelih > likelih0) {
			...
			if (FMAX(fabs(theta[0]-theta0[0]), fabs(theta[1]-theta0[1])) < TOL_) {
				break;
			} else {
				theta0[0] = theta[0];
				theta0[1] = theta[1];
